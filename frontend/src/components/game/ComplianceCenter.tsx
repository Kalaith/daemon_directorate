...
export const ComplianceCenter: React.FC = () => {
  ...
  const canCompleteTask = (task: ComplianceTask) => {
    if (task.requirements.resourceCost) {
      ...
      if (cost.bureaucraticLeverage && resources.bureaucraticLeverage < cost.bureaucraticLeverage) return false;
      if (cost.rawMaterials && resources.rawMaterials < cost.rawMaterials) return false;
    }
    if (task.requirements.daemonsRequired && daemons.filter(d => d.isActive).length < task.requirements.daemonsRequired) {
      return false;
    }
    return true;
  };
    ...
    <Card className="bg-gradient-to-br from-red-900 to-pink-900">
                  ...
                  <div key={task.id} className={`p-4 rounded-lg border-2 ${getTaskColor(task)}`}>
                    ...
                    {task.requirements && (
                      <div className="mt-3 p-2 bg-gray-800 rounded">
                        ...
                        {task.requirements.daemonsRequired && (
                          <p className="text-xs text-gray-300">• {task.requirements.daemonsRequired} daemons required</p>
                        )}
                        {task.requirements.resourceCost && (
                          <div className="text-xs text-gray-300">
                            {Object.entries(task.requirements.resourceCost).map(([resource, cost]) => (
                              <p key={resource}>• {cost} {resource}</p>
                            ))}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="mt-3 p-2 bg-red-900/30 rounded">
                      ...
                      {task.penalties.moraleLoss && (
                        <p className="text-xs text-red-300">• -{task.penalties.moraleLoss} morale to all daemons</p>
                      )}
                      {task.penalties.resourceFines && (
                        <div className="text-xs text-red-300">
                          {Object.entries(task.penalties.resourceFines).map(([resource, fine]) => (
                            <p key={resource}>• -{fine} {resource} fine</p>
                          ))}
                        </div>
                      )}
                      {task.penalties.daemonReassignment && (
                        <p className="text-xs text-red-300">• One daemon reassigned to another department</p>
                      )}
                    </div>
                  </div>
                ))}
                    ...
                    <div key={task.id} className={`p-4 rounded-lg border-2 ${getTaskColor(task)}`}>
                      <div className="flex items-start justify-between">
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-300 mb-2">{task.description}</p>
                            <div className={`text-xs ${daysRemaining <= 2 ? 'text-yellow-400' : 'text-blue-400'}`}>
                      ...
                      {task.requirements && (
                        <div className="mt-3 p-2 bg-gray-800 rounded">
                          <p className="text-xs font-medium text-gray-400 mb-1">Requirements:</p>
                          {task.requirements.daemonsRequired && (
                            <p className={`text-xs ${daemons.filter(d => d.isActive).length >= task.requirements.daemonsRequired ? 'text-green-300' : 'text-red-300'}`}>
                              • {task.requirements.daemonsRequired} daemons required ({daemons.filter(d => d.isActive).length} available)
                            </p>
                          )}
                          {task.requirements.resourceCost && (
                            <div className="text-xs">
                              {Object.entries(task.requirements.resourceCost).map(([resource, cost]) => {
                                ...
                                return (
                                  <p key={resource} className={hasEnough ? 'text-green-300' : 'text-red-300'}>
                      ...
                      {!canComplete && (
                        <div className="mt-3 p-2 bg-red-900/30 rounded">
                          <p className="text-xs text-red-400">Cannot complete: Insufficient resources or daemons</p>
                        </div>
                      )}
                    </div>
                  );
      ...
      <div className="mt-6 p-3 bg-gray-800 rounded-lg">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          ...
          <div>
            <div className="text-lg font-bold text-green-300">{complianceTasks.filter(t => t.completed).length}</div>
