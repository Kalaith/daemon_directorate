...
export const createComplianceSlice: StateCreator<ComplianceSlice, [], [], ComplianceSlice> = (
  set,
  get
) => ({
  // Initial state
  ...
  getOverdueTasks: (currentDay: number) => {
    ...
    return complianceTasks.filter(task => !task.completed && currentDay >= task.deadline);
  },
