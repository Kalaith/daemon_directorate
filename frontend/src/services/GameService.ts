...
class GameService implements GameServiceInterface {
  ...
  recruitDaemon = withErrorHandling(async (daemonId: string): Promise<Daemon> => {
    ...
    return candidate;
  }, 'GameService.recruitDaemon', ErrorCategory.BUSINESS_RULE);

  dismissDaemon = withErrorHandling(async (daemonId: string): Promise<boolean> => {
    const state = this.store.getState();
    ...
    return true;
  }, 'GameService.dismissDaemon', ErrorCategory.BUSINESS_RULE);

  getDaemonStats = withErrorHandling(async (daemonId: string): Promise<Daemon | null> => {
    const state = this.store.getState();
    return state.daemons.find((d: Daemon) => d.id === daemonId) || null;
  }, 'GameService.getDaemonStats', ErrorCategory.SYSTEM);

  executeMission = withErrorHandling(async (planetId: string, teamIds: string[]): Promise<MissionResult> => {
    const state = this.store.getState();
    ...
    const team = teamIds.map(id => state.daemons.find((d: Daemon) => d.id === id)).filter(Boolean);
    ...
    return result;
  }, 'GameService.executeMission', ErrorCategory.BUSINESS_RULE);

  getPlanetInfo = withErrorHandling(async (planetId: string): Promise<Planet | null> => {
    const state = this.store.getState();
    return state.planets.find((p: Planet) => p.id === planetId) || null;
  }, 'GameService.getPlanetInfo', ErrorCategory.SYSTEM);

  getAvailableMissions = withErrorHandling(async (): Promise<Mission[]> => {
    const state = this.store.getState();
    return state.availableProceduralMissions || [];
  }, 'GameService.getAvailableMissions', ErrorCategory.SYSTEM);

  getResources = withErrorHandling(async (): Promise<GameResources> => {
    const state = this.store.getState();
    return { ...state.resources };
  }, 'GameService.getResources', ErrorCategory.SYSTEM);

  spendResources = withErrorHandling(async (costs: Partial<GameResources>): Promise<boolean> => {
    const state = this.store.getState();
    ...
    return true;
  }, 'GameService.spendResources', ErrorCategory.BUSINESS_RULE);

  addResources = withErrorHandling(async (gains: Partial<GameResources>): Promise<void> => {
    const state = this.store.getState();
    await state.addResources(gains, 'Service layer operation');
  }, 'GameService.addResources', ErrorCategory.SYSTEM);

  saveGameState = withErrorHandling(async (): Promise<boolean> => {
    try {
  ...
  }, 'GameService.saveGameState', ErrorCategory.SYSTEM);

  loadGameState = withErrorHandling(async (): Promise<boolean> => {
    try {
      return await this.store.getState().loadGame();
    } catch {
      return false;
    }
  }, 'GameService.loadGameState', ErrorCategory.SYSTEM);

  resetGame = withErrorHandling(async (): Promise<void> => {
    await this.store.getState().resetGame();
  }, 'GameService.resetGame', ErrorCategory.SYSTEM);
}
