              ...
              {bloodlines.map(bloodline => {
                ...
                return (
                  <div
                    ...
                    onClick={() => setSelectedBloodline(selectedBloodline === bloodline ? null : bloodline)}
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className={`font-semibold ${status.isExtinct ? 'text-gray-400' : 'text-white'}`}>
                          {bloodline}
                          {status.isExtinct && ' (Extinct)'}
                        </h4>
                        <div className="text-sm text-gray-400 space-y-1">
                          <p>Generation {status.totalGenerations}</p>
                          <p>{status.totalStories} stories • {status.legends} legends</p>
                            ...
                            {legacy.legends.map((legend, index) => (
                              <div key={index} className="p-2 bg-yellow-600/20 rounded mb-2">
                                ...
                                <div className="text-xs text-gray-300 mt-1">
                                  Effects: {legend.effects.map(e => `${e.type} +${e.value}`).join(', ')}
                                </div>
                              </div>
                            ))}
                          </div>
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                      story.category === 'heroic' ? 'bg-blue-600/30 text-blue-300' :
                      story.category === 'tragic' ? 'bg-red-600/30 text-red-300' :
                      story.category === 'absurd' ? 'bg-purple-600/30 text-purple-300' :
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          ...
          <div>
            <div className="text-lg font-bold text-yellow-300">
              {Object.values(legacyBook).reduce((sum, legacy) => sum + legacy.legends.length, 0)}
            </div>
