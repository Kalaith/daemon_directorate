...
import { COMPONENT_VARIANTS, ANIMATIONS, LAYOUT_PRESETS } from '../../theme/tokens';
...
export const ProgressBar: React.FC<ProgressBarProps> = ({
  ...
  const getBarColor = () => {
    if (variant === 'health') {
      return percentage >= 70 ? COMPONENT_VARIANTS.status.health.good :
             percentage >= 40 ? COMPONENT_VARIANTS.status.health.warning :
             COMPONENT_VARIANTS.status.health.critical;
    }
    if (variant === 'morale') {
      return percentage >= 70 ? COMPONENT_VARIANTS.status.morale.good :
             percentage >= 40 ? COMPONENT_VARIANTS.status.morale.warning :
    ...
    <div className={className}>
      {showLabel && (
        <div className="flex justify-between text-sm mb-1">
          ...
          <span className="font-semibold text-neutral-900">{value}{max === 100 ? '%' : `/${max}`}</span>
        </div>
      )}
...
export const StatusIndicator: React.FC<StatusIndicatorProps> = ({
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      {value !== undefined && (
        <span className={`text-sm font-semibold ${variant === 'lifespan' ? getStatusColor() : 'text-neutral-900'}`}>
          {value}
        </span>
      )}
    </div>
...
export const Container: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <div className={`${LAYOUT_PRESETS.container} ${className}`}>
    {children}
  </div>
);

export const FlexBetween: React.FC<{ children: React.ReactNode; className?: string }> = ({
  children,
  className = '',
}) => (
  <div className={`${LAYOUT_PRESETS.flexBetween} ${className}`}>
    {children}
  </div>
);

export const GridResponsive: React.FC<{ children: React.ReactNode; className?: string }> = ({
  children,
  className = '',
}) => (
