...
export const EndgameModal: React.FC = () => {
  ...
  const ending = ENDING_SCENARIOS[endgameState.managementStyle as keyof typeof ENDING_SCENARIOS];
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-black/80 flex justify-center items-center z-50">
      <div className="bg-surface rounded-lg p-8 w-11/12 max-w-2xl max-h-96 overflow-y-auto border border-border">
        <div className={`text-center p-8 rounded-lg bg-gradient-to-br ${getEndingColor(endgameState.managementStyle)} text-white mb-6`}>
          <div className="text-6xl mb-4">{getEndingIcon(endgameState.managementStyle)}</div>
        ...
        <div className="space-y-4">
          <div className="p-4 bg-gray-800 rounded-lg">
            <h3 className="font-semibold text-yellow-300 mb-3">🎉 Unlocked Content:</h3>
            {ending.unlocks.length > 0 ? (
              <ul className="text-sm text-gray-300 space-y-1">
                {ending.unlocks.map((unlock: string, index: number) => (
                  <li key={index}>• {unlock.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}</li>
                ))}
              </ul>
            ) : (
              <p className="text-sm text-gray-400">No additional content unlocked</p>
            )}
          </div>

          {endgameState.permanentBonuses.length > 0 && (
            <div className="p-4 bg-purple-900/30 rounded-lg">
              <h3 className="font-semibold text-purple-300 mb-3">⭐ Prestige Bonuses:</h3>
              <div className="space-y-2">
                {endgameState.permanentBonuses.map(bonus => (
                  <div key={bonus.id} className="p-2 bg-purple-800/30 rounded">
                    <h4 className="font-medium text-purple-200">{bonus.name}</h4>
        ...
        <div className="flex gap-4 mt-8">
          ...
          >
            ...
            <div className="text-sm opacity-90 mt-1">Start new game with bonuses</div>
          </button>
        ...
        <div className="mt-6 p-4 bg-gray-800 rounded-lg text-center">
          <p className="text-sm text-gray-300">
            Thank you for playing Daemon Directorate! Your corporate legacy will live on in infernal history.
          </p>
        </div>
      </div>
    </div>
