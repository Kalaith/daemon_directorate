...
import type { Daemon, CorporateEvent, ComplianceTask, DaemonLegacy, LegacyStory } from '../types/game';
...
export class DashboardService {
  static calculateDashboardStats(
    ...
    const latestEvent = corporateEvents.length > 0
    ...
    const recentStories = hallOfInfamy.slice(-DAEMON_BALANCE.MAX_RECENT_STORIES);
  ...
  static getHealthStatusClass(health: number): string {
    if (health >= DAEMON_BALANCE.HEALTH_THRESHOLDS.GOOD) return 'text-green-600';
    if (health >= DAEMON_BALANCE.HEALTH_THRESHOLDS.WARNING) return 'text-orange-600';
    return 'text-red-600';
  }

  static getMoraleStatusClass(morale: number): string {
    if (morale >= DAEMON_BALANCE.MORALE_THRESHOLDS.GOOD) return 'text-teal-600';
    if (morale >= DAEMON_BALANCE.MORALE_THRESHOLDS.WARNING) return 'text-orange-600';
    return 'text-red-600';
  }
